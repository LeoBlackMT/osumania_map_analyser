...
- **junk file 显式化**：MinaCalc 对"荒唐密度"谱面（例如 1 秒 150+ 行）不报错，而是打印 `skipping junk file` 并返回**全 0 技能值**。`calc.js` 现在输出 `junkFile`（要求 ≥32 行且所有展示技能值**四舍五入到 0.00**，即绝对值 < 0.005——显示层只保留两位小数，0.004 与 0 对用户没有区别）与 `rowCount`；`analysis.js` 把 Etterna 段渲染为 **"MSD unavailable (MinaCalc junk file)"**，并让 MSD 胶囊与分隔符显示 `-`/`--`，不再显示 `0.00`。
