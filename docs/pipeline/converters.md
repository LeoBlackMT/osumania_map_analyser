...
- `js/parser/mcToOsuConverter.js` — `.mc`（Malody chart JSON）→ `.osu` v14（移植自 [mc_to_osu.py](https://github.com/LeoBlackMT/nonebot_plugin_osumania_toolkit)（MIT，源自 Jakads/malody2osu））
- `js/parser/vendor/simfile-parser/` — simfile-parser v0.9.0（MIT）vendored 副本，来源与本地补丁见 `NOTICE.md`

两种转换器的输出都直接进入既有 `OsuFileParser`（管线零改动）。固定参数：OD=9 / HP=8 / AR=5 / Mode=3。
...
运行（Node 22+ 无类型翻转问题；插件目录无 `package.json type:module`，Node 需按 ESM 解析 `.js`）：

```
node --experimental-detect-module tests/run-converter-tests.mjs
```

- 测试脚本与合成生成器**均不进仓库**（`tests/` 整体 gitignored，纯本地验证）；合成样本由 `tests/generate-fixtures.mjs` 生成（可随时重建）。
- **真实谱面样本仅本机私有**：放入 `tests/fixtures/real/`（目录 gitignored），存在即自动冒烟；示例来源（本机）：`D:\Games\Etterna\Songs`、`D:\Steam\steamapps\common\MalodyV\chart` 下的 `.sm`/`.mc`。
