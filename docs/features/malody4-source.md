...
| 设置单例 | `ReadProcessMemory`：`module_base + 0x8311C4` → 用户设置单例 `S`（判定档 `S+0x9C`、`user_mods` `S+0xD0`）；`module_base + 0x8310C4` → 本局 play-config `P`（`P+0x14` / `P+0x00`） | **判定档与变速位的主来源**：游戏里一改立刻生效——改判定 / 改变速位的 UI 处理器写的正是这两个对象 |
...
**`reason` 闭集（13 个字面量，逐字引用 `desktop/docs/CONTRACT.md` §8；不在此处改写）**：`chart-not-indexed`、`chart-unknown-identity`、`process-not-found`、`multiple-instances`、`access-denied`、`bad-read`、`target-mismatch:pe_timestamp_mismatch`、`target-mismatch:file_size_mismatch`、`target-mismatch:pe_header_out_of_range`、`target-mismatch:unknown`、`root-not-configured`、`no-library`、`platform-unsupported`。其中 `root-not-configured` **只表示整条解析链走完仍为 `None`**（见 §5），不是"用户没配"的同义词；两个谱面级未命中原因的分界是**"试完没有"**——`chart-not-indexed` = 索引里**暂时**没查到（重建尝试还没试完：库可能仍在建、文件可能刚落地），`chart-unknown-identity` = 这个身份键**已查过、重建尝试也用完仍解析不出来**（本次会话不再为它重建，见 §7.1）。新字面量是闭集的**向后兼容扩展**（`reason` 对页面是不透明诊断串，页面只存不分支），故契约仍是 v3。两者对帧形态与卡片行为完全相同（hidden 记录、卡片保留上一张谱面）。
...
- **Settings singletons** via `ReadProcessMemory`: `module_base + 0x8311C4` → the user-settings singleton `S` (judge at `S+0x9C`, `user_mods` at `S+0xD0`); `module_base + 0x8310C4` → the per-play `P` (judge at `P+0x14`, `user_mods` at `P+0x00`). This is the **primary source for the judge level and the speed mod**: an in-game change takes effect immediately, because the UI handlers for those two settings write exactly these two objects.
...
**`reason` closed set** (13 literals quoted from `desktop/docs/CONTRACT.md` §8, not paraphrased here): `chart-not-indexed`, `chart-unknown-identity`, `process-not-found`, `multiple-instances`, `access-denied`, `bad-read`, `target-mismatch:pe_timestamp_mismatch`, `target-mismatch:file_size_mismatch`, `target-mismatch:pe_header_out_of_range`, `target-mismatch:unknown`, `root-not-configured`, `no-library`, `platform-unsupported`. `root-not-configured` means **the whole resolution chain produced nothing**, not "the user did not configure it". The two chart-level miss reasons are split by **whether the retries are exhausted**: `chart-not-indexed` = the index does not have it *yet* (rebuild attempts not exhausted: the library may still be building, the file may have just landed), `chart-unknown-identity` = this identity was looked up and still fails to resolve **after the rebuild attempts were exhausted** (no further rebuilds for it this session, see §7.1). The new literal is a **backwards-compatible extension** of the closed set (`reason` is an opaque diagnostic string to the page, which stores it and never branches on it), so the contract stays at v3. Both produce the same frame shape and the same card behaviour (a hidden record, the card keeping the previous chart).
