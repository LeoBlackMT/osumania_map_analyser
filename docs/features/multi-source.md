...
Malody V（编辑器插件 WriteFile `<base>_mma_request.json` → 壳 ≤1Hz 扫 chart/
...
- 契约：`desktop/docs/CONTRACT.md`（版本 3，八型帧：新增 `malody4_selection` 与 `requestId n{seq}` 前缀；九领域：帧/身份/requestId 状态机/应答两来源/settings 在线只读离线双向/封面白名单/state 周期/contract 终态；皮肤状态文件章节已标注废弃）。
...
- Bridge contract: `desktop/docs/CONTRACT.md` (v3, eight frame types).
