...
8. **aleju03 与 Mixed 的低段 LN 接管**：`mixedEstimator.js` 在算出 `sunnyParts.ln` 后判断该 LN 标签是否以 `<` 开头（`intervalLookup` 的下限标记，如 `< LN 5 mid`），是则调用 aleju03 并把其 LN 标签作为 LN 半；判定要求 `estDiff` 确实含 `||`（避免把 RC 侧的 `<` 边界误当 LN 下限）。aleju03 任何失败都保留原表结果；实测 102 张 LN 谱中 19 张触发、其余谱面逐字不变（详见 [breakings/2026-09-19](../breakings/2026-09-19-aleju03-ln-estimator-lowband-routing.md)）。
9. **aleju03 的移植边界与输出契约**：计算层逐字移植 mania-hub 的 `features.ts` / `ln.ts`（参考数据表 127 行程序化提取，未手工转录）；**未移植** `offGridRowShare`（需 timing 表，且 LN 路径的距离/floors/回归都不读它）；段落模型与动作学臂不在移植范围内。独立选择该算法时 `estDiff` **只含 LN 难度**且使用区间表同款 tier 词（`LN 7 mid/high`），`numericDifficulty` / `numericDifficultyHint` 为 null；**谱面完全不含长条（LN% = 0）时返回 `Unknown difficulty`**（参考邻域模型对无长条谱面没有语义，回归兜底会给出误导性数字），诊断 `aleju03Ln.reason = "no-ln-content"`；其余 4K 谱面都会给出判决（`forceLn` 跳过源的 LN 候选门；候选门不过则走 `ln-pressure` 回归兜底）；Mixed 的低段接管保持源的候选门语义（`forceLn=false`）。**算法名必须同时存在于三处**才可选：`settings.json` 的 options、`config.js` 的 `APP_CONFIG.options.estimatorAlgorithm`、以及 `settingsParser.js normalizeEstimatorAlgorithmValue` 的别名白名单（后者不认识该名会静默回退默认 `Mixed`）。该参考表末段含上游为评测标注谱面折入的锚点，且结构 floor 使用了窄区间判据——因此它在本仓库只用于 LN 标签，档位边界（`< LN 5`）之外的行为不做改动。
