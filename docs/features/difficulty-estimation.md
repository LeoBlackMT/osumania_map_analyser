...
9. **aleju03 的移植边界与输出契约**：计算层逐字移植 mania-hub 的 `features.ts` / `ln.ts`（参考数据表 127 行程序化提取，未手工转录）；**未移植** `offGridRowShare`（需 timing 表，且 LN 路径的距离/floors/回归都不读它）；段落模型与动作学臂不在移植范围内。独立选择该算法时 `estDiff` **只含 LN 难度**且使用区间表同款 tier 词（`LN 7 mid/high`），`numericDifficulty` / `numericDifficultyHint` 为 null，任意 4K 谱面都会给出判决（`forceLn` 跳过源的 LN 候选门；候选门不过则走 `ln-pressure` 回归兜底）；Mixed 的低段接管保持源的候选门语义（`forceLn=false`）。该参考表末段含上游为评测标注谱面折入的锚点，且结构 floor 使用了窄区间判据——因此它在本仓库只用于 LN 标签，档位边界（`< LN 5`）之外的行为不做改动。
